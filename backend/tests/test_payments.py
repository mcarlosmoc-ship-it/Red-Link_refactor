...
def test_payment_clears_debt_and_sets_service_active(client, db_session, seed_basic_data):
    ...
    db_session.refresh(client_model)
    assert Decimal(client_model.debt_months) == Decimal("0")
    assert Decimal(client_model.paid_months_ahead) == Decimal("1")
    assert client_model.service_status == models.ServiceStatus.ACTIVE
