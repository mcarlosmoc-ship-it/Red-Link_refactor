...
export default function PointOfSalePage() {
  ...
  const eligiblePeriods = useMemo(() => {
    ...
    const previous = current ? addMonthsToPeriod(current, -1) : null
  ...
  const {
    receipts: selectedClientReceipts,
    isLoading: isLoadingSelectedClientReceipts,
    refetch: refetchSelectedClientReceipts,
  } = useClientReceipts({ clientId: selectedClient?.id, limit: 8, enabled: Boolean(selectedClient) })
  ...
  const toggleServiceBillingSelection = (service) => {
    if (!selectedClient) {
      setFormError('Selecciona un cliente antes de cobrar servicios.')
      focusClientSearch()
      return
    }
    ...
    updateCart((current) => {
      ...
      const unitPrice = Number(service.effectivePrice ?? service.price ?? CLIENT_PRICE)
      const name = service.name ?? 'Servicio mensual'
      ...
      return [
        ...current,
        {
          ...
          productId: null,
          name,
          category: service.category ?? 'Servicio mensual',
          unitPrice: unitPrice > 0 ? unitPrice : CLIENT_PRICE,
          quantity: 1,
          type: 'monthly-service',
  ...
  const duplicateServiceReceiptMap = useMemo(() => {
    if (!activePeriodKey || !selectedClientReceipts.length) {
      return {}
    }

    return selectedClientReceipts.reduce((acc, receipt) => {
      if (receipt.period === activePeriodKey && receipt.serviceId) {
        acc[String(receipt.serviceId)] = receipt
      }
      return acc
    }, {})
      ...
      <header className="space-y-3">
        <div className="flex flex-col gap-2 lg:flex-row lg:items-center lg:justify-between">
          ...
          <div className="flex flex-wrap items-center gap-2 text-xs text-slate-500">
            <span className="inline-flex items-center gap-1 rounded-full bg-blue-50 px-3 py-1 font-semibold uppercase tracking-wide text-blue-700">
              <CreditCard className="h-3.5 w-3.5" aria-hidden /> Caja activa
            </span>
            <span className="inline-flex items-center gap-1 rounded-full bg-emerald-50 px-3 py-1 font-semibold uppercase tracking-wide text-emerald-700">
              <BadgeCheck className="h-3.5 w-3.5" aria-hidden /> Sincronizado
            </span>
            <span className="inline-flex items-center gap-1 rounded-full bg-slate-100 px-3 py-1 font-semibold uppercase tracking-wide text-slate-700">
              <Printer className="h-3.5 w-3.5" aria-hidden /> Lista para imprimir
            </span>
          </div>
        </div>
        <div className="grid gap-3 rounded-lg border border-slate-200 bg-slate-50 p-4 shadow-sm sm:grid-cols-3">
          <div className="flex items-start gap-2 text-sm text-slate-700">
            <Ticket className="mt-0.5 h-4 w-4 text-blue-600" aria-hidden />
            <div>
              <p className="text-xs uppercase tracking-wide text-slate-500">Ticket en curso</p>
              <p className="font-semibold">Ticket #001 · Mostrador</p>
            </div>
          </div>
          <div className="flex items-start gap-2 text-sm text-slate-700">
              ...
              <div className="rounded-lg border border-slate-200 bg-white p-3 shadow-sm">
                ...
                <div className="mt-2 flex flex-wrap items-center gap-3 text-[11px] text-slate-500">
                  <span className="inline-flex items-center gap-1 rounded-full bg-blue-50 px-2 py-1 font-semibold uppercase tracking-wide text-blue-700">
                    <RefreshCw className="h-3 w-3" aria-hidden /> Vista ligera inspirada en Eleventa
                  </span>
                  <span>Usa Enter para agregar al ticket rápidamente.</span>
                  <span className="hidden sm:inline">Escala mejor en monitores grandes con estilo de escritorio.</span>
                </div>
              </div>

              <div className="overflow-hidden rounded-lg border border-slate-200 bg-white shadow-sm">
                <div className="flex items-center justify-between bg-slate-50 px-4 py-2 text-[11px] font-semibold uppercase tracking-wide text-slate-600">
                  <span>Catálogo de productos</span>
                  <span className="flex items-center gap-1 text-slate-500">
                    <ShoppingBag className="h-3.5 w-3.5" aria-hidden /> Doble clic agrega al ticket
                  </span>
                </div>
                {isLoadingProducts ? (
                  <div className="flex items-center gap-3 px-4 py-6 text-sm text-slate-500">
                    <ShoppingBag className="h-4 w-4 animate-spin" aria-hidden /> Cargando catálogo…
                  </div>
                ) : filteredSalesProducts.length === 0 ? (
                  <div className="space-y-1 px-4 py-6 text-sm text-slate-500">
                  ...
                  <table className="min-w-full divide-y divide-slate-200 text-sm">
                    <thead className="bg-slate-50">
                      <tr>
                        ...
                        <th scope="col" className="px-4 py-2 text-right font-semibold text-slate-600">
                          Acción
                        </th>
                      </tr>
                    </thead>
                      ...
                      {filteredSalesProducts.map((product) => (
                        ...
                        >
                          <td className="px-4 py-3">
                            ...
                            {product.sku ? (
                              <p className="text-[11px] uppercase tracking-wide text-slate-400">SKU {product.sku}</p>
                            ) : null}
                          </td>
                          <td className="px-4 py-3 text-sm font-semibold text-slate-900">{peso(product.unitPrice)}</td>
                          <td className="px-4 py-3 text-sm text-slate-700">
                            {product.stockQuantity !== null ? (
                              ...
                              >
                                {product.stockQuantity > 0 ? 'Stock' : 'Sin stock'} · {product.stockQuantity}
                              </span>
                            ) : (
                              <span className="inline-flex items-center gap-1 rounded-full bg-slate-100 px-2 py-1 text-[11px] font-semibold uppercase tracking-wide text-slate-700">
                                <BadgeCheck className="h-3 w-3" aria-hidden /> Ilimitado
                              </span>
                            )}
                          </td>
                          <td className="px-4 py-3 text-right">
                            <div className="flex items-center justify-end gap-2">
                              ...
                              <Button type="button" size="sm" onClick={() => addProductToCart(product)}>
                    ...
                    <div className="space-y-5 p-5">
                      <div className="flex items-center justify-between gap-3">
                        <div>
                          ...
                          <p className="text-lg font-bold text-slate-900">CARRITO ({cartItems.length} producto(s))</p>
                        </div>
                        <Button
                          type="button"
                          variant="ghost"
                      ...
                      <div className="relative">
                        ...
                        <input
                          ...
                          className="w-full rounded-2xl border border-slate-200 px-12 py-4 text-lg font-medium text-slate-800 shadow-sm focus:border-blue-400 focus:outline-none focus:ring-2 focus:ring-blue-100"
                        />
                      </div>
                            ...
                            >
                              ...
                              <div className="flex items-center gap-2">
                                <Button
                                  type="button"
                                  variant="outline"
                                  className="h-11 w-11 rounded-full border-slate-200 text-lg font-bold text-slate-800 hover:bg-slate-100"
                                  onClick={() => decreaseItemQuantity(item.id)}
                                  aria-label={`Disminuir cantidad de ${item.name}`}
                                >
                                  <Minus className="h-4 w-4" aria-hidden />
                                </Button>
                                <span className="w-14 text-center text-lg font-semibold text-slate-900">
                                  {toInputValue(item.quantity, 2)}
                                </span>
                                <Button
                                  type="button"
                                  variant="outline"
                              ...
                              <div className="w-28 text-right text-lg font-bold text-slate-900">
                      ...
                      <div className="sticky bottom-0 space-y-4 rounded-2xl border border-slate-200 bg-slate-50/80 p-4">
                        ...
                        <div className="flex items-center justify-between rounded-xl bg-white px-4 py-3 shadow-sm">
                          <div>
                            <p className="text-[11px] uppercase tracking-wide text-slate-500">Total a pagar</p>
                        ...
                        <Button
                          type="button"
                          className="w-full rounded-2xl bg-emerald-600 py-4 text-lg font-bold text-white shadow-md hover:bg-emerald-700"
                          disabled={isSubmittingSale || !checkoutGuard.canCheckout}
                          onClick={handleOpenPaymentModal}
                        >
                          <Receipt className="mr-2 h-5 w-5" aria-hidden /> Pagar
                        </Button>
                      </div>
                    </div>
                  </div>
