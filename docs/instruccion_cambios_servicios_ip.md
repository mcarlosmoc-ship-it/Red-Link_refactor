...
- Gestión de IP separada en tablas dedicadas (`ip_pools`/`ip_reservations`) vinculadas a `client_services` e inventario, sin fijar la IP en el contrato (el servicio referencia `ip_reservation_id`).
...
2. Integrar la asignación de IP al flujo de registro/edición de servicios: reservar del pool correspondiente y asociar a `client_services`/`inventory_items` vía `ip_reservation_id`.
