...
try {
    ...
    Invoke-Step 'Iniciando backend y frontend' {
        $backendCommand = "Set-Location -Path `"$backendDir`"; if (Test-Path '.venv\\Scripts\\Activate.ps1') { . .\\.venv\\Scripts\\Activate.ps1 }; uvicorn app.main:app --reload --host 0.0.0.0 --port 8000 --app-dir `"$backendDir`""
        Start-Process powershell -ArgumentList '-NoExit', '-Command', $backendCommand -WindowStyle Normal

        $frontendCommand = "Set-Location -Path `"$repoRoot`"; npm run dev"
